...
const { auth, requireAdminOrEditor } = require('../middlewares/auth.cjs');
...
router.get('/', auth, requireAdminOrEditor, async (req, res) => {
  try {
    ...
    queryStr += ` ORDER BY m.${finalSortBy} ${finalSortOrder.toUpperCase()} LIMIT ${parseInt(limit, 10)} OFFSET ${parseInt(offset, 10)}`;
