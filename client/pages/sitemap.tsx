...
export const getServerSideProps: GetServerSideProps = async () => {
  try {
    // Fetch categories
    const categoriesResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/v2/categories`);
    const categoriesData = await categoriesResponse.json();
    
    // Fetch recent posts
    const postsResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/v2/posts?limit=20&sort=latest`);
