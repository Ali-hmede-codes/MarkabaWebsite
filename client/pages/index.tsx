...
const HomePage: NextPage<HomePageProps> = ({ posts, categories, error }) => {
  ...
  const [technologyPosts, setTechnologyPosts] = useState<Post[]>([]);
  ...
  // Fetch Sports posts from the sports category
  const fetchSportsPosts = async () => {
    try {
      setSportsLoading(true);
      const response = await fetch('/api/v2/categories/sports?include_posts=true&posts_limit=4&posts_page=1');
      if (response.ok) {
        const data = await response.json();
        if (data.success && data.data?.posts) {
          setSportsPosts(data.data.posts);
        }
      }
    } catch (error) {
  ...
  // Fetch Technology posts from the technology category
  const fetchTechnologyPosts = async () => {
    try {
      setTechnologyLoading(true);
      const response = await fetch('/api/v2/categories/technology?include_posts=true&posts_limit=4&posts_page=1');
      if (response.ok) {
        const data = await response.json();
        if (data.success && data.data?.posts) {
          setTechnologyPosts(data.data.posts);
        }
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // Fetch Sports posts separately
    fetchSportsPosts();
    
    // Fetch Technology posts separately
        ...
        <div className="bg-white min-h-screen" dir="rtl">
          ...
          <div className="container mx-auto responsive-padding">
            ...
            <section className="mb-32" style={{ backgroundColor: '#f8faff' }}>
              <div className="p-6 lg:p-6 px-0 sm:px-6">
                <div 
                  className="mb-6 sm:mb-8 text-right flex justify-between items-center px-6 sm:px-0"
                  style={{
                    background: 'linear-gradient(to right, transparent 0%, transparent 100%), linear-gradient(to right, #3B82F6, #2563EB) 0% 100% / 100% 3px no-repeat'
                  }}
                >
                ...
                {khasLoading ? (
                  ...
                  <div className="space-y-6">
                    ...
                    {/* Mobile Layout */}
                    <div className="lg:hidden space-y-4 px-6 sm:px-0">
                      {/* Large Post */}
                      {khasPosts[0] && (
                ...
                <div className="space-y-6">
                  ...
                  <div className="hidden lg:block">
                    <div className="grid grid-cols-12 gap-6 h-96">
                      ...
                      <div className="col-span-7 h-full order-0">
                        {sportsPosts[0] && (
                          <Link href={`/post/${sportsPosts[0].slug}`} className="block bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 cursor-pointer h-full">
                            <div className="w-full h-full flex flex-col">
                              {/* Image */}
                              <div className="flex-grow overflow-hidden relative">
                                <LazyImage
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent flex flex-col justify-end p-6">
                                  {/* Category */}
                                  {getCategoryName(sportsPosts[0].category_id) && (
                                    <span className="inline-block bg-white text-black text-sm px-3 py-1 rounded mb-3 w-fit font-medium">
                                      {getCategoryName(sportsPosts[0].category_id)}
                                    </span>
                                  )}
                                  ...
                                  {/* Title */}
                                  <h3 className="text-white font-bold text-xl mb-3 leading-tight" style={{ fontFamily: 'Alexandria, sans-serif' }}>
                                    {sportsPosts[0].title_ar || sportsPosts[0].title}
                                  </h3>
                                  
                                  {/* Meta */}
                                  <div className="flex items-center text-white/80 text-sm">
                                    <FiCalendar className="inline ml-2" size={14} />
                                    {getRelativeTime(sportsPosts[0].created_at)}
                                  </div>
                                </div>
                              </div>
                            </div>
                      ...
                      {/* Small Posts - Left side (order-1) using video mobile design */}
                      <div className="col-span-5 h-full order-1 space-y-4">
                        {sportsPosts.slice(1, 4).map((post, index) => (
                          <Link key={post.id} href={`/post/${post.slug}`} className="block bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 cursor-pointer h-32">
                            <div className="flex h-full">
                              {/* Image - 4:3 ratio */}
                              <div className="w-32 flex-shrink-0">
            ...
            {/* تــــكـــنـــولـوجـيــا Section */}
            <section className="mb-32">
              <div 
                className="mb-6 sm:mb-8 text-right flex justify-between items-center"
                style={{
              ...
              >
                ...
                >
                  تــــكـــنـــولـوجـيــا
                </h2>
                
                <Link href="/category/technology" className="flex items-center gap-2 text-black hover:text-gray-700 transition-colors duration-200 text-sm sm:text-base font-medium">
                  <FiArrowLeft className="w-4 h-4" />
                  <span>الــــمـــزيــــد</span>
                </Link>
              </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="hidden lg:block">
                    <div className="grid grid-cols-12 gap-6 h-96">
                      ...
                      <div className="col-span-7 h-full order-0">
                        {technologyPosts[0] && (
                          <Link href={`/post/${technologyPosts[0].slug}`} className="block bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 cursor-pointer h-full">
                            <div className="w-full h-full flex flex-col">
                              {/* Image */}
                              <div className="flex-grow overflow-hidden relative">
                                <LazyImage
                                ...
                                <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent flex flex-col justify-end p-6">
                                  {/* Category */}
                                  {getCategoryName(technologyPosts[0].category_id) && (
                                    <span className="inline-block bg-white text-black text-sm px-3 py-1 rounded mb-3 w-fit font-medium">
                                      {getCategoryName(technologyPosts[0].category_id)}
                                    </span>
                                  )}
                                  ...
                                  {/* Title */}
                                  <h3 className="text-white font-bold text-xl mb-3 leading-tight" style={{ fontFamily: 'Alexandria, sans-serif' }}>
                                    {technologyPosts[0].title_ar || technologyPosts[0].title}
                                  </h3>
                                  
                                  {/* Meta */}
                                  <div className="flex items-center text-white/80 text-sm">
                                    <FiCalendar className="inline ml-2" size={14} />
                                    {getRelativeTime(technologyPosts[0].created_at)}
                                  </div>
                                </div>
                              </div>
                            </div>
                      ...
                      {/* Small Posts - Left side (order-1) using video mobile design */}
                      <div className="col-span-5 h-full order-1 space-y-4">
                        {technologyPosts.slice(1, 4).map((post, index) => (
                          <Link key={post.id} href={`/post/${post.slug}`} className="block bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 cursor-pointer h-32">
                            <div className="flex h-full">
                              {/* Image - 4:3 ratio */}
                              <div className="w-32 flex-shrink-0">
