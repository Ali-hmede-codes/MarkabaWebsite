...
const LastNewsManagement: React.FC = () => {
  ...
  const fetchNews = async () => {
    try {
      ...
      if (data.success) {
        ...
        toast.error('Failed to load last news');
      }
    } catch (error) {
      toast.error('Error fetching last news');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch(`/api/last-news/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await response.json();
  ...
  return (
    <AdminLayout>
      <div className="p-6">
        <h1 className="text-2xl font-bold mb-4">Manage Last News</h1>
