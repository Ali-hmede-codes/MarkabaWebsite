                  ...
                  <div key={`${news.id}-${news.isBreaking ? 'breaking' : 'last'}`} className="">
                    {news.isBreaking ? (
                      <div className="flex items-start gap-3">
                        <span className="text-blue-600 font-medium whitespace-nowrap" style={{ fontSize: '12px' }}>{timeAgo(news.created_at)}</span>
                      ...
                      <Link href={`/last-news/${news.slug || news.id}`}>
                        <div className="flex items-start gap-3 hover:bg-gray-50 p-2 rounded-lg transition-colors">
                          <span className="text-blue-600 font-medium whitespace-nowrap" style={{ fontSize: '12px', fontFamily: 'Alexandria, sans-serif' }}>{timeAgo(news.created_at)}</span>
                          <span className="text-gray-400">|</span>
                          <div className="flex-1">
