        ...
        <div className="flex flex-col gap-4 sm:gap-6 mb-4 sm:mb-6">
          ...
          <Link href={`/post/${latestPosts[0].slug}`} className="block rounded-xl overflow-hidden transition-all duration-300 hover:shadow-xl cursor-pointer relative border border-gray-200 hover:border-gray-300">
            <div className="aspect-video w-full relative">
              <OptimizedImage 
                ...
                alt={latestPosts[0].title_ar || latestPosts[0].title || 'صورة المقال'} 
                fill
          ...
          <div className="flex flex-col md:flex-row gap-3 md:gap-4">
            {latestPosts.slice(1, 3).map(post => (
              <Link key={post.id} href={`/post/${post.slug}`} className="flex gap-3 p-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer border border-gray-200 hover:border-gray-300">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="font-semibold text-sm text-gray-900 mb-1 line-clamp-2 leading-tight">{post.title_ar}</h4>
